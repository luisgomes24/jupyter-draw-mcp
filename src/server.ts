...
function loadJupyterInstructions(): string {
  return fsSync.readFileSync(path.join(PROMPTS_DIR, "jupyter-instructions.md"), "utf-8");
}
...
let _cachedJupyterInstructions: string | null = null;
function getJupyterInstructions(): string {
  if (!_cachedJupyterInstructions) {
    _cachedJupyterInstructions = loadJupyterInstructions();
  }
  return _cachedJupyterInstructions;
}
...
export function registerTools(server: McpServer, distDir: string, store: CheckpointStore): void {
  ...
  server.registerTool(
    ...
    async (): Promise<CallToolResult> => {
      readMeCalled = true;
      const jupyterRules = getJupyterInstructions();
  ...
  server.registerTool(
    ...
    async (): Promise<CallToolResult> => {
      return { content: [{ type: "text", text: getJupyterInstructions() }] };
    },
  );
